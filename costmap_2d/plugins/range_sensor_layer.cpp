#include <costmap_2d/range_sensor_layer.h>
#include <boost/algorithm/string.hpp>
#include <pluginlib/class_list_macros.h>
#include <angles/angles.h>

PLUGINLIB_EXPORT_CLASS(costmap_2d::RangeSensorLayer, costmap_2d::Layer)
...
void RangeSensorLayer::onInitialize()
{
  ...
  for (unsigned int i = 0; i < topic_names.size(); i++)
  {
    ...
    else
    {
      ...
      topic_name="/sensors/range"; // Tsompania
  ...
  dsrv_ = new dynamic_reconfigure::Server<costmap_2d::RangeSensorLayerConfig>(nh);
  dynamic_reconfigure::Server<costmap_2d::RangeSensorLayerConfig>::CallbackType cb = boost::bind(
...
void RangeSensorLayer::get_deltas(double angle, double *dx, double *dy)
{
    ...
    if(ta==0)
        ...
        *dx = resolution_ / ta;  // where resolution_ is the resolution of the costmap_2d

    // z=copysign(x,y), z gets the magnitude of x and sign of y
...
void RangeSensorLayer::reconfigureCB(costmap_2d::RangeSensorLayerConfig &config, uint32_t level)
{
  phi_v_ = config.phi;
...
{
  // The max angle is half the field of view (aka cone_angle)
  ...
  // blocks until a transform is possible
  if(!tf_->waitForTransform(global_frame_, in.header.frame_id,
        in.header.stamp, ros::Duration(0.8)) ) {
     ROS_ERROR_THROTTLE(1.0, "Range sensor layer can't transform from %s to %s at %f",
        global_frame_.c_str(), in.header.frame_id.c_str(),
        in.header.stamp.toSec());
     return;
  }
  // transforms the stamped_in to the target frame and stores it to stamped_out
  // target_frame, stamped_in, stamped_out
  tf_->transformPoint (global_frame_, in, out);

  // here ox and oy contain 0,0 since they were not given any value, since in and out are empty
  double ox = out.point.x, oy = out.point.y;

  // set the range we calculated to x and transform it to the map frame
